...
export async function generateQuotePDF(
  ...
  const priceIsVatInclusive = /magma\s*10\s*kw.*flue\s*kit.*special/i.test(input.productName);
  ...
  // ---------- Footer terms ----------
  const isMagmaSpecial = priceIsVatInclusive;
  ensureSpace(6);
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text(
    isMagmaSpecial
      ? "PAYMENT TERMS: 100% ON ORDER (PROGRESS GROUP QUOTE)"
  ...
  if (includeInstallEstimate) {
    ...
    if (isMagmaSpecial) {
      ensureSpacePage2(10);
      doc.setFont("helvetica", "bold").setFontSize(10);
      doc.text("PAYMENT TERMS: 75% DEPOSIT ON THIS ESTIMATE", margin, py);
      py += 5;
      doc.setFont("helvetica", "normal").setFontSize(9);
      doc.text(
        "75% of the installation estimate is payable to Progress Installations before installation; balance on completion.",
        margin,
        py,
      );
      py += 8;
